...
if (isset($_POST['badgeNumber']) && isset($_POST['checkoutTime'])) {
    ...
    $result = mysqli_query($link, "SELECT * FROM visitor WHERE badge_number=$badgeNumber AND checkout_time IS NULL ORDER BY id DESC LIMIT 1");
    ...
    $result = mysqli_query($link, "UPDATE visitor SET checkout_time=\"$checkoutTime\" WHERE id=$column_id");

    // check if row inserted or not
    if ($result) {
        // successfully inserted into database
        $response["success"] = 1;
        $response["message"] = "Visitor checkedout successfully.";
